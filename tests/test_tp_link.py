...
def test_entry_point_url_valid():
    scraper = TPLinkScraper(create_logger(), scrape_entry_url=DOWNLOAD_URL_GLOBAL)
...
def test_entry_point_url_invalid():
    with pytest.raises(WebDriverException):
        ...
        scraper = TPLinkScraper(create_logger(), scrape_entry_url=invalid_entry_point_url)
        scraper.driver.get(scraper.scrape_entry_url)


def test_scrape_product_metadata():
    scraper = TPLinkScraper(create_logger(), max_products=10)
