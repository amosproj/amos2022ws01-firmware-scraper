...
def test_entry_point_url_valid():
    scraper = QnapScraper(create_logger(), scrape_entry_url=HOME_URL, headless=True)
...
def test_entry_point_url_invalid():
    with pytest.raises(WebDriverException):
        ...
        scraper = QnapScraper(create_logger(), scrape_entry_url=invalid_entry_point_url, headless=True)
        scraper.driver.get(scraper.scrape_entry_url)


def test_scrape_product_metadata():
    scraper = QnapScraper(create_logger(), headless=True, max_products=2)
