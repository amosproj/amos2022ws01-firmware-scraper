...
def test_entry_point_url_valid():
    scraper = EngeniusScraper(create_logger_old(), scrape_entry_url=HOME_URL, headless=True)
...
def test_entry_point_url_invalid():
    with pytest.raises(WebDriverException):
        ...
        scraper = EngeniusScraper(create_logger_old(), scrape_entry_url=invalid_entry_point_url, headless=True)
        scraper.driver.get(scraper.scrape_entry_url)


def test_scrape_product_metadata():
    scraper = EngeniusScraper(create_logger_old(), headless=True, max_products=5)
