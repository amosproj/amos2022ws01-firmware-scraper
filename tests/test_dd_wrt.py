...
def test_entry_point_url_valid():
    scraper = DDWRTScraper(create_logger_old(), scrape_entry_url=DOWNLOAD_URL)
...
def test_entry_point_url_invalid():
    with pytest.raises(WebDriverException):
        ...
        scraper = DDWRTScraper(create_logger_old(), scrape_entry_url=invalid_entry_point_url)
        scraper.driver.get(scraper.scrape_entry_url)


def test_scrape_product_metadata():
    scraper = DDWRTScraper(create_logger_old(), max_products=10)
