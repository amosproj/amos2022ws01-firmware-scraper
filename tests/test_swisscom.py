...
def test_entry_point_url_valid():
    scraper = SwisscomScraper(create_logger(), scrape_entry_url=DOWNLOAD_URL_EN)
...
def test_entry_point_url_invalid():
    with pytest.raises(WebDriverException):
        ...
        scraper = SwisscomScraper(create_logger(), scrape_entry_url=invalid_entry_point_url)
        scraper.driver.get(scraper.scrape_entry_url)


def test_scrape_product_metadata():
    scraper = SwisscomScraper(create_logger())
