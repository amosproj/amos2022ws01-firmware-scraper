...
class SynologyScraper(Scraper):
    ...
    def _get_MD5_checksum(self) -> Optional[str]:
        ...
        """
        try:
            el = self.driver.find_element(By.XPATH, value=SELECTOR_MD5)
        ...
        try:
            ...
            try:
                ...
                html_str = self.driver.find_element(
                    By.CSS_SELECTOR,
                    "#release_notes_content > div.container.margin_bottom80.margin_top80",
                ).get_attribute("innerHTML")
                start, end = "<div>", "</div>"
                release_date = (
                    html_str[html_str.find(start) + len(start) : html_str.rfind(end)]
                    .split("\n")[0]
                    .replace("</div>", "")
                    .replace("(", "")
                    .replace(")", "")
    ...
    def scrape_metadata(self) -> list[dict]:
        ...
        for product_line in product_catalog.keys():
            ...
            for product in product_catalog[product_line]:
                ...
                (
                    tmp_metadata_dict["release_date"],
                    tmp_metadata_dict["version"],
                ) = self._get_release_date_and_fw_version()
