...
class LinksysScraper:
    ...
    def scrape_metadata_from_product_urls(self, product_urls):
        ...
        for product_url in product_urls[30:]:
            ...
            #self.driver.get("https://www.linksys.com/support-article?articleNum=148365")
            ...
            for i in range(len(version_clicks)):
                ...
                for j in range(len(titles)):
                    #print(titles[j].text)
                    if titles[j].text not in ["Firmware ", "Firmware"]:
                        continue
                    info_p = version_parents[i].find_elements(By.TAG_NAME, 'p')
                    for p in info_p:
                        if p.text == "Release Notes":
                            info_p.remove(p)
                    #next_element = title.next
                    download_link = info_p[i].find_element(By.TAG_NAME, 'a').get_attribute("href")
